...
int checkFreq(int x,int y,struct food f[],int s,struct mine m[],int r)
{
	if(checkH(x,y))
	return 1;
...
void getFood(struct food *f,int s,struct mine m[])
{
int x,y;
do{
x=(rand()%(M));	
y=(rand()%(N));
}while(checkFreq(x,y,f,s,m,-1));
...
void getMine(struct food *f,struct mine m[],int r)
{
int x,y;
do{
x=(rand()%(M));	
y=(rand()%(N));
}while(checkFreq(x,y,f,-1,m,r));
...
struct snake* start(struct snake *head,struct food f[],struct mine m[])
{
 if(head==NULL)
...
int checkMine(struct snake* head,struct mine m[])
{
	
	int i;
	for(i=0;i<mc;i++)
	{
	if(head->x==m[i].x && head->y==m[i].y)
	{
		return i;			
	}	
...
void gameOver(struct snake* head,struct mine m[])
{
	int flag=1;
	int a=checkMine(head,m);
...
struct snake* shift(struct snake* head,int x,int y,struct food f[],struct mine m[])
{
...
struct snake* move(struct snake* head,struct food f[],struct mine m[])
{
	char mov;
